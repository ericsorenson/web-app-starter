...
const {
  ...
  deleteAllTodos,
  createTodo,
} = require('./InMemoryPersistenceGateway');
...
describe('inMemoryStorageClient', () => {
  ...
  describe('readTodos()', () => {
    ...
    it('returns todos when todos have been created', () => {
      createTodo({
        todoData: { description: 'do good things' },
  ...
  describe('createTodos()', () => {
    it('creates todo', () => {
      deleteAllTodos();
      createTodo({
        todoData: { description: 'do more good things' },
        applicationContext: mockApplicationContext,
      });
      const todos = readAllTodos();
  ...
  describe('deleteAllTodos()', () => {
    it('deletes all todos', () => {
      ...
      createTodo({
        todoData: { description: 'do even more good things' },
        applicationContext: mockApplicationContext,
      });
      const someTodos = readAllTodos(mockApplicationContext);
      assert.deepStrictEqual(someTodos, [
        { description: 'do more good things' },
        { description: 'do even more good things' },
      ]);
      deleteAllTodos(mockApplicationContext);
