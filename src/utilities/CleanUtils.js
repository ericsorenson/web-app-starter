...
function validateRequestData({ data, dataSchema, applicationContext }) {
    if (!data || !dataSchema || !applicationContext) {
        throw new Error("Hey programmer - at least one of your request parameters is shit.")
    }
    const validationResult = applicationContext.getJsonValidator().validateJson(data, dataSchema);
